...
CASE WHEN(LEN(ed.site) > 10)
		...
		END AS siteEditora,
CASE 
	WHEN(est.quantidade < 5) THEN 'Produto em Ponto Pedido'
	WHEN(est.quantidade > 5 AND est.quantidade <= 10) THEN  'Produto Acabando'
	ELSE 'Estoque Suficiente'
END as [status do produto]
FROM estoque est
INNER JOIN editora ed
ON ed.codigo = est.codEditora
GROUP BY est.nome, est.quantidade, ed.site
