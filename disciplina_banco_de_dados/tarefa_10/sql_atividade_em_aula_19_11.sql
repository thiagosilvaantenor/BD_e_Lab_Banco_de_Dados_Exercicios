...
SELECT * FROM produto
SELECT * FROM pedido
--- 1. Consultar a quantidade, valor total e valor total com desconto (25%) dos itens comprados
-- par Maria Clara.
SELECT p.quantidade, (prod.valor_unitario * p.quantidade) as valorTotal,
(prod.valor_unitario * p.quantidade) - ((prod.valor_unitario * p.quantidade) * 0.25) AS valorTotalComDesconto
FROM pedido p
INNER JOIN produto prod
ON prod.codigo = p.cod_prod
INNER JOIN cliente c
ON c.codigo = p.cod_cli
...
SELECT COUNT(*) as clientesAcimaQuarenta
FROM cliente c
...
WHERE forn.atividade = 'Brinquedo' OR forn.atividade = 'Chocolate'
-- 7. Consultar nome e desconto de 25% no preço dos produtos que custam menos de R$50,00
SELECT prod.nome, (prod.valor_unitario - (prod.valor_unitario * 0.25) ) AS valorComDesconto
from produto prod
WHERE prod.valor_unitario < 50.00

-- 8. Consultar nome e aumento de 10% no preço dos produtos que custam mais de R$100,00
SELECT prod.nome, (prod.valor_unitario + (prod.valor_unitario * 0.10) ) AS valorComDesconto
from produto prod
WHERE prod.valor_unitario > 100

-- 9. Consultar desconto de 15% no valor total de cada produto da venda 99001.
SELECT (prod.valor_unitario * p.quantidade) - ((prod.valor_unitario * p.quantidade) * 0.15) AS valorTotalComDesconto
from produto prod
INNER JOIN pedido p
ON prod.codigo = p.cod_prod
...
SELECT DISTINCT p.codigo, c.nome,
DATEDIFF(YEAR, c.data_nasc, GETDATE() ) AS idadeAtual
FROM pedido p
INNER JOIN cliente c
ON c.codigo = p.cod_cli
...
ON f.codigo = p.cod_forn
GROUP BY p.valor_unitario, f.nome
HAVING p.valor_unitario IN
			(
				SELECT MAX(p.valor_unitario)
				from produto p
			)


--12. Consultar a média dos valores cujos produtos ainda estão em estoque
SELECT AVG(p.valor_unitario) AS [media val prod ainda em estoque]
FROM produto p
...
p.valor_unitario, (p.valor_unitario * ped.quantidade) AS [valor total]
FROM cliente c
INNER JOIN pedido ped
ON c.codigo = ped.cod_cli
INNER JOIN produto p
ON p.codigo = ped.cod_prod
WHERE c.nome = 'Maria Clara'
...
SELECT DISTINCT DATEDIFF(DAY,ped.previsao_ent, '15-03-2023') as [dias de atraso]
FROM pedido ped
INNER JOIN cliente c
ON c.codigo = ped.cod_cli
WHERE c.nome = 'Maria Clara'
...
SELECT DISTINCT CONVERT(VARCHAR(10),DATEADD(DAY, 3, ped.previsao_ent),103) AS [nova data de entrega]
FROM pedido ped
INNER JOIN cliente c
ON c.codigo = ped.cod_cli
